...
import type { Unit, Player, GameEvent, UnitDeathEvent, SkillCastEvent, Obstacle } from '../shared/types.js';
...
if (!canvasEl) throw new Error('gameCanvas not found');
...
socket.on('joined', (d: { selfId: string; selfName: string; players: Player[]; units: Unit[]; obstacles?: Obstacle[] }) => {
  // 补齐 obstacles（服务器已发送，此处仅为类型兼容）
  const payload = d as unknown as { selfId: string; selfName: string; players: Player[]; units: Unit[]; obstacles: Obstacle[] };
  if (!payload.obstacles) payload.obstacles = [];
  state.init(payload);
  hideLoading();
});

socket.on('gameState', (d: { players: Player[]; units: Unit[] }) => {
  state.updateState(d);
});

// 游戏事件（单位死亡、技能施放等）→ EventBus 广播
socket.on('gameEvent', (evt: GameEvent) => {
  EventBus.emit(evt);
});

// 默认订阅：单位死亡 → 提示
EventBus.on<UnitDeathEvent>('unitDeath', (evt) => {
...
EventBus.on<SkillCastEvent>('skillCast', (_evt) => {
  // 预留后续播放特效
});
...
interface PointerRecord {
  id: number; sx: number; sy: number; startSx: number; startSy: number;
  lastSx: number; lastSy: number; movedScreenDist: number;
}
...
// ---------- 事件层系统 ----------
// 三层事件分层，避免 UI 事件与游戏内指针事件互相冲突
// Game：游戏内操作（选单位、摇杆、地图拖拽等）
// GameUI：UI 操作（技能栏、HUD 等）
// GameMenu：菜单操作（技能说明气泡等）
// 规则：越靠上的层 (GameMenu > GameUI > Game）越先生效，下层不处理
type EventLayer = 'game' | 'gameui' | 'gamemenu';

// 检查 pointerdown 事件是否发生在某个非 game/gameui/gamemenu 层 DOM 上
function eventHitsLayer(el: HTMLElement | null, layer: EventLayer): boolean {
  if (!el) return false;
  let cur: HTMLElement | null = el;
  while (cur) {
    if (cur.dataset && cur.dataset.layer === layer) return true;
...
function eventHitsNonGameLayer(targetElem: EventTarget | null): boolean {
  const el = targetElem as HTMLElement | null;
  return eventHitsLayer(el, 'gamemenu') || eventHitsLayer(el, 'gameui');
}

// ---------- 底部提示 ----------
let hintTimeout: number | null = null;
function showBottomHint(text: string, durationMs: number = 1800): void {
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (eventHitsNonGameLayer(e.target)) return;
...
canvas.addEventListener('pointermove', (e) => {
  ...
  if (mode === 'JOYSTICK') {
    const movedFromStart = Math.sqrt(
      (sx - rec.startSx) * (sx - rec.startSx) + (sy - rec.startSy) * (sy - rec.startSy)
    );
    // 移动超过一点 → 激活摇杆（让用户看到在拖）
...
function ptsFromPointers(recs: PointerRecord[]): { sx: number; sy: number }[] {
...
function onPointerEnd(e: PointerEvent, cancel: boolean): void {
  e.preventDefault();
  const rec = activePointers.get(e.pointerId);
  if (rec) activePointers.delete(e.pointerId);
...
function getActiveSkillDef(unit: Unit): { id: string; name: string; type: 'direction' | 'point'; cooldownMs: number; chargeMultiplier: number; hpCostPct?: number; radius?: number; damage?: number; description: string } | null {
  const slot = unit.skills?.[unit.activeSkillIndex ?? 0];
  if (!slot) return null;
  return (SKILL_DEFS as Record<string, { id: string; name: string; type: 'direction' | 'point'; cooldownMs: number; chargeMultiplier: number; hpCostPct?: number; radius?: number; damage?: number; description: string }>)[slot.defId] || null;
}

function releaseDirectionSkill(): void {
  if (!joy.active || !joy.unitId) return;
  ...
  if (dist >= JOY_RADIUS_MIN_FIRE && unit) {
    ...
    if (unit.hp - hpCost <= 0) {
      ...
      joy.active = false; joy.unitId = null;
      return;
    }
...
interface PinchState {
  startDist: number; startMidSx: number; startMidSy: number;
  startCamX: number; startCamY: number; startZoom: number;
  lastMidSx: number; lastMidSy: number;
}
...
function handlePinch(pts: { sx: number; sy: number }[]): void {
  ...
  const anchor = screenToWorldRaw(pinchState.startMidSx, pinchState.startMidSy,
    pinchState.startCamX, pinchState.startCamY, pinchState.startZoom, width, height);
  camZoom = newZoom;
...
function pinchEnd(): void {
  pinchState = null;
}
function screenToWorldRaw(sx: number, sy: number, cX: number, cY: number, cZ: number, w: number, h: number): { x: number; y: number } {
  return {
    x: (sx - w / 2) / cZ + cX,
    y: (sy - h / 2) / cZ + cY,
  };
...
// ---------- 小地图移动 ----------
function minimapMoveTo(sx: number, sy: number): void {
  if (minimap.w === 0) return;
...
function drawObstacles(): void {
  ctx.save();
  for (const ob of state.obstacles as unknown as { x: number; y: number; size: number }[]) {
...
function drawJoystick(): void {
  ...
  const wy = rp ? rp.ry : unit.y;
  const scr = worldToScreen(wx, wy);
  joy.baseScreenX = scr.x; joy.baseScreenY = scr.y;
...
function drawPointSkillAimCircle(): void {
  const sel = state.getSelectedUnit();
  if (!sel) return;
  const activeDef = getActiveSkillDef(sel);
  if (!activeDef || activeDef.type !== 'point') return;
  if (!activeDef.radius) return;
  if (!state.isUnitStopped(sel.id)) return;
  ...
  let visible: [number, number][] = [[-Math.PI, Math.PI]];
  for (const occ of occluded) {
    const nextVisible: [number, number][] = [];
    for (const vis of visible) {
      ...
      // full occlusion
      if (occ[0] <= vis[0] && occ[1] >= vis[1]) continue;
  ...
  if (occluded.length > 0) {
    ...
    ctx.beginPath();
    const merged: [number, number][] = occluded;
...
function ensureSkillBar(): HTMLElement {
  let el = document.getElementById('skillBar') as HTMLElement | null;
  if (el) return el;
...
// ---------- 技能说明气泡 ----------
let skillTooltipEl: HTMLDivElement | null = null;
let activeTooltipIndex: number | null = null;

function ensureSkillTooltip(): HTMLDivElement {
  ...
  window.addEventListener('pointerdown', (e) => {
    ...
    if (skillTooltipEl.style.display === 'none') return;
    // 如果点击在气泡或任意 .skill-info 上 → 不关闭
    const target = e.target as Node | null;
    if (!target) return;
    if (skillTooltipEl.contains(target)) return;
    if ((target as Element).classList?.contains?.('skill-info')) return;
    hideSkillTooltip();
  });
...
function buildSkillTooltipHTML(
  def: { name: string; icon: string; description: string; type: string; cooldownMs: number; hpCostPct?: number; chargeMultiplier?: number; damage?: number; radius?: number },
  cdMs: number,
): string {
  ...
  if (typeof def.damage === 'number') extras.push(`基础伤害 ${def.damage}`);
  if (typeof def.radius === 'number') extras.push(`作用半径 ${def.radius}`);
  if (extras.length > 0) {
...
function toggleSkillTooltip(index: number, anchor: HTMLElement): void {
  const sel = state.getSelectedUnit();
  if (!sel || !sel.skills || sel.skills[index] === undefined) return;
  const slot = sel.skills[index];
  const def = (SKILL_DEFS as Record<string, { name: string; icon: string; description: string; type: string; cooldownMs: number; hpCostPct?: number; chargeMultiplier?: number; damage?: number; radius?: number }>)[slot.defId];
  if (!def) return;
  ...
  if (left + bubbleWidth > screenW - margin) left = screenW - margin - bubbleWidth;
  if (top < margin) {
    // 上方空间不足，显示在下方
    top = anchorRect.bottom + 10;
    if (top + bubbleHeight > screenH - margin) top = Math.max(margin, screenH - margin - bubbleHeight);
  }
...
function renderSkillBarDOM(): void {
  ...
  while (bar.childElementCount < needed) {
    ...
    btn.onpointerdown = (e) => {
      e.stopPropagation();
      e.preventDefault();
      const idx = Number((btn as HTMLButtonElement).dataset.skillIndex);
      if (Number.isNaN(idx)) return;
      const current = state.getSelectedUnit();
      if (!current || !current.skills || idx >= current.skills.length) return;
  ...
  for (let i = 0; i < needed; i++) {
    const btn = bar.children[i] as HTMLButtonElement;
    const slot = sel.skills[i];
    const def = (SKILL_DEFS as Record<string, { name: string; icon: string; description: string; cooldownMs: number; hpCostPct?: number; type: string; chargeMultiplier?: number; damage?: number; radius?: number }>)[slot.defId];
    ...
    // 标题 + 图标
    const infoDot = btn.querySelector('.skill-info') as HTMLSpanElement | null;
    btn.innerHTML = `
    ...
    if (infoDot) btn.appendChild(infoDot);
    ...
    // 冷却覆盖
    let overlay = btn.querySelector('.cd-overlay') as HTMLDivElement | null;
    if (!overlay) {
...
function drawMinimap(): void {
  ...
  const W = GAME_CONSTANTS.WORLD_BOUND;
  const toMMx = (wx: number) => minimap.x + ((wx - (-W)) / (2 * W)) * mmW;
  const toMMy = (wy: number) => minimap.y + ((wy - (-W)) / (2 * W)) * mmH;
