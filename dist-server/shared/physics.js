...
export function segmentIntersectsRect(x1, y1, x2, y2, cx, cy, size) {
    const half = size / 2;
    const minX = cx - half;
    const maxX = cx + half;
    const minY = cy - half;
...
export function pointDistanceToSegment(px, py, x1, y1, x2, y2) {
    const dx = x2 - x1;
    const dy = y2 - y1;
...
export function lineOfSight(fromX, fromY, toX, toY, obstacles, units, unitRadius = GAME_CONSTANTS.UNIT_RADIUS) {
    for (let i = 0; i < obstacles.length; i++) {
...
function raycastSegment(x1, y1, x2, y2, cx, cy, size) {
...
function raycastCircle(x1, y1, x2, y2, cx, cy, r) {
...
export function raycastToFirstHit(fromX, fromY, toX, toY, obstacles, units, unitRadius = GAME_CONSTANTS.UNIT_RADIUS) {
    let bestT = Infinity;
    let hit = false;
    for (let i = 0; i < obstacles.length; i++) {
        const ob = obstacles[i];
